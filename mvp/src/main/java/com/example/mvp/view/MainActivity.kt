...
import com.example.mvp.presenter.MainContract
...
class MainActivity : AppCompatActivity(), MainContract.View {
    ...
    // 1) Model         - MVC 패턴의 Model과 같은 역할 : 데이터를 저장하는 클래스, 데이터를 제어하는 코드를 가진 클래스들
    // 2) View          - 사용자가 볼 화면을 구현하는 목적의 코드가 작성되는 파일들
    // 3) Presenter     - View 와 Model 사이에서 연결하는 역할, interface 로 역할을 정해놓기에 특정 객체를 만들어 참조하여 결합되는 것을 방지함

    // ** 주요 제작 특징 : view 와 presenter 가 해야할 작업들을 미리 interface를 통해 설계해 놓음으로써 작업의 역할 구분이 명확하게 보임.
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnLoad.setOnClickListener { presenter.clickedLaod() }
...
// MVP의 장점
// 1. MVC 처럼 데이터를 제어하는 코드가 Activity 나 Fragment 에 없어서 간결하고 재사용 가능함
// 2. Activity 를 view 역할로만 사용하기에 보다 명확하게 역할 구분이 됨.
// 3. model 역할을 하는 클래스 안에 어떤 view 참조변수도 없기에 완전분리가 가능함.
// 4. view 역할 클래스안에 model 참조변수가 없기에 model의 기능이 변경되어도 영향이 없음
// 5. Presenter 클래스가 참조하는 것을 특정 Activity나 Fragment로 정하지않고 view 인터페이스로 정하였기에 변경이 용이함
