...
class ItemModel constructor(val context:Context) { // 데이터를 제어하기 위해 Context 능력이 필요한 경우가 많아서 주 생성자를 이용해서 전달받기

    // 1. controller 역할의 클래스로부터 데이터를 전달받아 SharedPreferences 에 데이터를 저장하는 기능
    ...
    // 2. sharedPreference 에서 데이터를 읽어와서 controller 에게 리턴해주는 기능
    fun loadData():Item{
